...
app.controller('messageCtrl', function ($scope, $routeParams, $sce, imageService, $rootScope, postService,blogutil) {
    ...
    var obj = null;
    var mBlogID, mPostID;
    if($routeParams.messageId != null){
        if($routeParams.messageId.split("-") != undefined && $routeParams.messageId.split("-").length ){
            mBlogID = $routeParams.messageId.split('-')[0];
            mPostID =  $routeParams.messageId.split('-')[1];
        }
    }
    ...
    if(typeof feedObj == "object"){
        if(feedObj.length == 0){
            imageService.getPosts(mBlogID,1).then(function(data){
                blogutil.parseFeed(data);
                feedObj = blogutil.getFeedObj();
                obj = blogutil.searchObjectArray(feedObj,"id",$routeParams.messageId);  
            })
        }
        obj = blogutil.searchObjectArray(feedObj,"id",$routeParams.messageId);
    }
